...
static void update_cea(double now) {
    ...
    // Accumulate overcharge to be refunded at each monitoring stage.
...
//Rate-reduction reservation 

static double compute_effective_rate(double reserved_energy, double q, double now) {
    double time_until = q - now;
    if (time_until <= 0.0) return 0.0;
    return energy_rate - reserved_energy / time_until;
}

static bool rate_allows_job(const SchedJob* job, double effective_rate) {
    if (job->walltime <= 0.0) return false;
    return (compute_job_energy(job) / job->walltime) <= effective_rate;
}
...
uint8_t batsim_edc_init(const uint8_t* data, uint32_t size, uint32_t flags) {
    ...
    if ((flags & (BATSIM_EDC_FORMAT_BINARY | BATSIM_EDC_FORMAT_JSON)) != flags) {
        printf("reducepc_shutdown: unknown flags, cannot initialize.\n");
        return 1;
    }
    ...
    printf("reducepc_shutdown: B=%.2e J, t_s=%.0f s, t_e=%.0f s, rate=%.2f W\n",
    ...
    for (unsigned int i = 0; i < nb_events; ++i) {
        ...
        switch (event->event_type()) {
            case fb::Event_BatsimHelloEvent:
                mb->add_edc_hello("reducepc_shutdown", "0.1.0");
                break;
