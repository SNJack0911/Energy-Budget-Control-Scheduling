...
// Off hosts drain at P_off (no overestimate needed; P_off is the actual value).
// On idle hosts drain at P̃_idle (overestimate); computing hosts' extra cost
// (P_comp - P_idle) is pre-charged at launch.
...
static void update_cea(double now) {
    ...
    uint32_t n_on      = platform_nb_hosts - n_off;
    uint32_t n_on_idle = n_on - n_comp;

    // Drain at overestimates (P̃_idle for on hosts, P_off for off hosts).
    C_ea += (energy_rate - n_on * P_IDLE_EST - n_off * P_OFF_EST) * dt;

    // Accumulate overcharge: computing hosts use (P̃_comp - P_comp_actual),
    // on-idle hosts use (P̃_idle - P_idle_actual), off hosts have no overcharge.
    pending_correction += n_comp    * (P_COMP_EST - P_COMP_ACTUAL) * dt
                        + n_on_idle * (P_IDLE_EST - P_IDLE_ACTUAL) * dt;
...
static std::list<SchedJob*>::iterator run_greedy_pass(double now) {
    ...
    while (it != queue.end()) {
        ...
        // Energy check uses current C_ea — wake up only if check passes
...
// If the queue head is blocked only on energy (has enough total hosts including
// off), schedule one targeted wakeup callback at the time C_ea will be sufficient.
// After do_shutdown() all idle hosts are off, so use the SHUTDOWN accrual rate.
    ...
    std::list<SchedJob*>::iterator first_job_it, double now)
{
    ...
    uint32_t total = static_cast<uint32_t>(free_hosts.size() + off_hosts.size());
    if (total < job->nb_hosts) return; // blocked on resources, not energy

    double needed = compute_job_energy(job);
    if (C_ea >= needed) return;

    // After do_shutdown, idle hosts are off; only n_comp hosts are on.
    uint32_t n_off_now = platform_nb_hosts - n_comp;
    double net_rate = energy_rate - n_comp * P_IDLE_EST - n_off_now * P_OFF_EST;
...
uint8_t batsim_edc_init(const uint8_t* data, uint32_t size, uint32_t flags) {
    ...
    if ((flags & (BATSIM_EDC_FORMAT_BINARY | BATSIM_EDC_FORMAT_JSON)) != flags) {
        printf("energybud_shutdown: unknown flags, cannot initialize.\n");
        return 1;
    }
    ...
    printf("energybud_shutdown: B=%.2e J, t_s=%.0f s, t_e=%.0f s, rate=%.2f W\n",
    ...
    for (unsigned int i = 0; i < nb_events; ++i) {
        ...
        switch (event->event_type()) {
            case fb::Event_BatsimHelloEvent:
                mb->add_edc_hello("energybud_shutdown", "0.1.0");
                break;
